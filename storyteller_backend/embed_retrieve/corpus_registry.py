...
class CorpusRegistry:
    ...
    def __init__(self, registry_file: Optional[str] = None):
        # Use settings to determine the correct path if not provided
        if registry_file is None:
            try:
                from config.settings import settings
                registry_file = str(settings.data_path / "corpus_registry.json")
            except ImportError:
                # Fallback to relative path (for backward compatibility)
                registry_file = "data/corpus_registry.json"
