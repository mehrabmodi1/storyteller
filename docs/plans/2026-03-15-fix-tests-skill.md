# Fix-Tests Skill — Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a Claude Code skill (`fix-tests`) that reads the latest test report, autonomously fixes all failing tests (bugs and unimplemented features), verifies each fix, then runs the full suite for regression safety.

**Architecture:** Single SKILL.md file containing a 4-phase procedure: read report → fix loop (fix, retest, retry) → full suite run → summary. The skill replicates test-running logic from `test-app` inline.

**Tech Stack:** Claude Code skills (SKILL.md), Playwright MCP, Bash (server lifecycle)

**Spec:** `docs/specs/2026-03-14-fix-tests-skill-design.md`

---

## Chunk 1: Create the Skill

### Task 1: Create the `fix-tests` skill file

**Files:**
- Create: `.claude/skills/fix-tests/SKILL.md`
...
Create `.claude/skills/fix-tests/SKILL.md` with the following exact content:
...
Read back `.claude/skills/fix-tests/SKILL.md` and confirm:
- Frontmatter has `name` and `description`
- All 4 phases are present
- Phase 1: report parsing with `✗ FAIL` pattern matching
- Phase 2: server readiness, context gathering, fix/feature implementation, backend restart, prerequisite replay, Playwright verification, 3-attempt retry, page reload
- Phase 3: full suite run with report format and comparison logic
- Phase 4: summary format with fixed/unresolved/regressions/modified files
- Permissions section present (no prompts for anything)
- Timeouts match `test-app` (60s streaming, 10s UI)
- "Tests are feature specs" philosophy stated

- [ ] **Step 4: Commit**

```bash
git add .claude/skills/fix-tests/SKILL.md
git commit -m "feat: add fix-tests Claude Code skill for autonomous test fixing"
```
